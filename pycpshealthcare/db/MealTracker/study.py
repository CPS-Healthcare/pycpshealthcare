...
class MealTrackerStudy:
    def __init__(self, connection):
        ...
        self.participants = participant_info.get_participants(studies="MealTracker").astype(
            "Participant"
        )
    ...
    def get_fitbit_at_meals(
        ...
        df_meals = []
        df_meals.append(
            self.get_meals_results(timestamp_start, timestamp_end, test_ids, "unwinded").astype(
                "dataframe", True
            )
        )
    ...
    def get_fitbit_at_meals_grouped(
        ...
        df_meals = []
        df_meals.append(
            self.get_meals_results(timestamp_start, timestamp_end, test_ids).astype(
                "dataframe", True
            )
        )
        df_meals = pd.concat(df_meals, axis=1)
