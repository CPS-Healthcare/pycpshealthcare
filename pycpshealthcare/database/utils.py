...
def generate_vector_magnitude_pipeline(id_match, timestamp_start=None, timestamp_end=None, coords=["accx", "accy", "accz"]):
    match_pipeline = {"$match": {**id_match}}
    if timestamp_start or timestamp_end:
        match_pipeline["$match"]["timestamp"] = {}
        if timestamp_start:
    ...
    pipeline=[
        ...
        {"$project": {
            ...
            "magnitude": {
                "$sqrt": {
                    "$sum": [{"$pow": [ { "$toDouble": f"$values.{coord}" }, 2 ]} for coord in coords]
                    }
                }
            }
        },
...
def generate_vector_stats_magnitude_pipeline(id_match, bin_size=60, bin_unit="minute", timestamp_start=None, timestamp_end=None, coords=["accx", "accy", "accz"]):
    ...
    group_pipe = {f"mag_{oper}": {f"${oper}": f"$magnitude"} for oper in ["avg", "min", "max"]}
    pipeline[2]["$group"].update(group_pipe)
    # project_pip = {f"{oper}_{i}": 1 for i in types for oper in ["avg", "min", "max"]}
    project_pip = {f"{oper}": {"magnitude": f"$mag_{oper}"}  for oper in ["avg", "min", "max"]}
