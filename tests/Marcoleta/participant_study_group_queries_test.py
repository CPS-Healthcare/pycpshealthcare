...
def test_fitbit_v2(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["Marcoleta"].get_fitbit_v2_results(
...
def test_fitbit_v2_metadata():
    ...
    for participant in participants:
        try:
            next(
                participant.studies_groups["Marcoleta"].get_fitbit_v2_metadata_results(
                    metadata_type="sleep"
                )
            )
            return
...
def test_holter(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["Marcoleta"].get_holter_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_autoreports(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["Marcoleta"].get_autoreports_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_fitbit_v2_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["Marcoleta"].get_fitbit_v2_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_holter_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["Marcoleta"].get_holter_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
