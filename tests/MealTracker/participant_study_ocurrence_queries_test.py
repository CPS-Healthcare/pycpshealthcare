    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["MealTracker"]) > 0:
                try:
                    next(participant.studies["MealTracker"][0].get_fitbit_results(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
...
def test_meals(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["MealTracker"]) > 0:
                try:
                    next(participant.studies["MealTracker"][0].get_meals_results(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_fitbit_at_meals(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["MealTracker"]) > 0:
                try:
                    next(participant.studies["MealTracker"][0].get_fitbit_at_meals(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
