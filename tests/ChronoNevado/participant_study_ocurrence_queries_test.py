    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_cpet_raw_data(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
...
def test_finapres_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_finapres_raw_data(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_finapres_data(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_finapres_data(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_spo2_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_spo2_raw_data(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_cpet_raw_data_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_cpet_raw_data_grouped(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_finapres_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_finapres_raw_data_grouped(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_finapres_data_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_finapres_data_grouped(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
...
def test_spo2_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            if len(participant.studies["ChronoNevado"]) > 0:
                try:
                    next(participant.studies["ChronoNevado"][0].get_spo2_raw_data_grouped(timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]))
                    return
                except StopIteration:
                    pass
        assert False, "Iterator is empty"
