...
def test_cpet_raw_data(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_cpet_raw_data(
...
def test_finapres_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_finapres_raw_data(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_finapres_data(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_finapres_data(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_spo2_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_spo2_raw_data(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_cpet_raw_data_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_cpet_raw_data_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_finapres_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_finapres_raw_data_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_finapres_data_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_finapres_data_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_spo2_rawdata(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["ChronoNevado"].get_spo2_raw_data_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
