...
def test_fitbit(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_fitbit_results(
...
def test_fitbit_v2(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_fitbit_v2_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_fitbit_v2_metadata():
    ...
    for participant in participants:
        try:
            next(
                participant.studies_groups["SanPedro"].get_fitbit_v2_metadata_results(
                    metadata_type="sleep"
                )
            )
            return
...
def test_holter(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_holter_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_alimentacion(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_alimentacion_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_freestyle_librelink(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_freestyle_librelink_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_inbody(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_inbody_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_patrones_minsal_2018(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_patrones_minsal_2018_results(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_fitbit_v2_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_fitbit_v2_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_holter_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_holter_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_alimentacion_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_alimentacion_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_freestyle_librelink_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_freestyle_librelink_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_inbody_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_inbody_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
...
def test_patrones_minsal_2018_grouped(params, expected):
    ...
    if expected == True:
        for participant in participants:
            try:
                next(
                    participant.studies_groups["SanPedro"].get_patrones_minsal_2018_results_grouped(
                        timestamp_start=params["ts_start"], timestamp_end=params["ts_end"]
                    )
                )
                return
